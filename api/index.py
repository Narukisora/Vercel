...
def index():
    ...
    total_files = getattr(total_count, "count", 0) or 0
...
def upload():
    ...
    if result.data:
        last_upload_at_str = result.data[0].get("last_upload_at")
        if last_upload_at_str:
            try:
...
def download(listing_id):
    ...
    if listing and listing.get("file_link", "").startswith("http"):
